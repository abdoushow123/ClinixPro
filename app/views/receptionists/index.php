...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 animate-fade-in">
    <div>
        <h1 class="text-3xl font-black text-slate-900 dark:text-white mb-1 tracking-tight">Receptionists Directory</h1>
        <p class="text-slate-500 dark:text-zinc-400 font-medium">Manage reception staff, applications, and approvals</p>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 items-stretch">
    <div class="glass-panel p-6 animate-slide-up flex flex-col hover:-translate-y-1 transition-transform duration-300" style="animation-delay: 0.1s;">
        <div class="flex items-center justify-between mb-4">
            <div class="w-12 h-12 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-xl flex items-center justify-center shadow-sm">
        ...
        <p class="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest">Total Receptionists</p>
    </div>
...
<div class="glass-panel overflow-hidden mb-8 animate-fade-in border-t-4 border-t-warning-500" style="animation-delay: 0.4s;">
    <div class="px-6 py-5 border-b border-slate-200/60 dark:border-zinc-700/50 bg-warning-50/50 dark:bg-warning-900/10 flex items-center justify-between">
        ...
        <span class="badge badge-warning"><?= count($pending_receptionists) ?> Pending</span>
    </div>
    <div class="table-wrapper border-0 shadow-none rounded-none">
        <table class="table-modern">
            <thead>
                <tr>
                    <th>Receptionist Profile</th>
                    <th>Email</th>
                    <th>Phone</th>
                    ...
                    <td>
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-gradient-to-br from-warning-500 to-warning-600 rounded-2xl flex items-center justify-center text-white font-bold shadow-sm">
                                <?= strtoupper(substr($receptionist['full_name'], 0, 1)) ?>
                            </div>
                            <span class="font-bold text-slate-900 dark:text-white"><?= htmlspecialchars($receptionist['full_name']) ?></span>
                        </div>
                    </td>
                    <td>
                        <div class="font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-1.5"><i class="bi bi-envelope text-slate-400"></i> <?= htmlspecialchars($receptionist['email']) ?></div>
                    </td>
                    <td>
                        <div class="font-medium text-slate-500 dark:text-zinc-400 flex items-center gap-1.5"><i class="bi bi-telephone text-slate-400"></i> <?= htmlspecialchars($receptionist['phone'] ?? 'N/A') ?></div>
                    </td>
                    <td class="text-right">
                        <div class="flex items-center justify-end gap-2">
                            <form method="POST" action="/receptionists/<?= $receptionist['id'] ?>/approve" class="inline" data-confirm="Approve this receptionist?">
                            ...
                            <form method="POST" action="/receptionists/<?= $receptionist['id'] ?>/reject" class="inline" data-confirm="Reject this receptionist?">
                                <input type="hidden" name="_token" value="<?= \App\Core\Security::generateCsrfToken() ?>">
...
<div class="glass-panel overflow-hidden animate-fade-in" style="animation-delay: 0.5s;">
    <div class="px-6 py-5 border-b border-slate-200/60 dark:border-zinc-700/50 flex items-center justify-between bg-white/40 dark:bg-zinc-800/40">
        <h3 class="font-bold text-lg text-slate-900 dark:text-white">Reception Staff</h3>
        <span class="badge badge-primary"><?= count($receptionists) ?> Staff Members</span>
    </div>
    <div class="table-wrapper border-0 shadow-none rounded-none">
        <table class="table-modern">
            <thead>
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="4" class="px-6 py-16 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <h4 class="text-lg font-bold text-slate-900 dark:text-white mb-1">No receptionists found</h4>
                            <p class="text-slate-500 dark:text-zinc-400 font-medium">There are currently no reception staff registered.</p>
                        </div>
                    </td>
                </tr>
                <?php else: ?>
                ...
                <tr>
                    <td>
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-2xl flex items-center justify-center font-bold text-lg shadow-sm border border-purple-200 dark:border-purple-800/50">
                                <?= strtoupper(substr($receptionist['full_name'], 0, 1)) ?>
                            </div>
                            <span class="font-bold text-slate-900 dark:text-white"><?= htmlspecialchars($receptionist['full_name']) ?></span>
                        </div>
                    ...
                    <td>
                        <span class="font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-1.5"><i class="bi bi-envelope text-slate-400"></i> <?= htmlspecialchars($receptionist['email']) ?></span>
                    </td>
                    <td>
                        <span class="font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-1.5"><i class="bi bi-telephone text-slate-400"></i> <?= htmlspecialchars($receptionist['phone'] ?? 'N/A') ?></span>
                    </td>
                    <td>
                        <?php if ($receptionist['registration_status'] === 'approved'): ?>
                            <span class="badge badge-success">
                                <span class="w-1.5 h-1.5 rounded-full bg-success-500 mr-1.5"></span>
                                Active
                            </span>
                        <?php elseif ($receptionist['registration_status'] === 'pending'): ?>
...
$displayTitle = 'Reception Staff';
$currentPath = '/receptionists';
